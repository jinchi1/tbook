...
public class NewPagerData {
    ...
    public static MyPost post;
    public static ArrayList<MyPost> postArr;
    ...
    public static NewPagerData NewPagerdataJSON(NewPagerData data, JSONObject json) throws  Exception{
        if(data == null) data = new NewPagerData();
        ...
        if(!json.isNull("isFollowing")) data.isFollowing = json.getString("isFollowing");
        if(!json.isNull("profile_photo")) data.profile_photo = json.getString("profile_photo");
        if(!json.isNull("user_name")) data.user_name = json.getString("user_name");
        ...
        if(!json.isNull("post")){
           ...
           // Log.i("testTag1234",obj.getString("comment"));
            ...
            if(!obj.isNull("photos")){
                ...
                post.photoArr = new ArrayList<MyPost.Photos>();
                MyPost.Photos photos;

                for (int j = 0 ; j < photoJsonArr.length();j++ ){
                    JSONObject Photo_obj = photoJsonArr.getJSONObject(j);

                    photos = new MyPost.Photos();
