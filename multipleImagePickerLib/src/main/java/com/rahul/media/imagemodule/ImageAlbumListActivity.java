    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView.setOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if(newState== AbsListView.OnScrollListener.SCROLL_STATE_IDLE) adapter.notifyDataSetChanged();
            }
        });
...
//                if (adapter == null) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Define.ENTER_ALBUM_REQUEST_CODE
           ...
           /* for(int j=0; j<data.getStringArrayListExtra(Define.INTENT_PATH).size();j++) {

                String a = data.getStringArrayListExtra(Define.INTENT_PATH).get(j);
                Log.i("tag",data.getStringArrayListExtra(Define.INTENT_PATH).get(j));
                try {
